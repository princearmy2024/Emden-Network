...
let CURRENT_VERSION = '4.66.11'; // Stand: 21.04.2026
...
const App = window.App = {
    ...
    async _refreshUserRole() {
        ...
        if (user.role === 'staff' || user.role === 'admin' || user.isStaff) {
            ...
            // Moderation Nav-Button IMMER einfuegen
            this._injectModButton();
        }
        if (user.role === 'admin') {
            document.querySelectorAll('.admin-only').forEach(el => el.classList.remove('hidden'));
        }
    ...
    _injectModButton() {
        // Alle moeglichen Slots probieren
        const slot = document.getElementById('modNavSlot');
        const existing = document.getElementById('modNavBtn');
        ...
        // Fallback: Vor dem Server-Nav-Item einfuegen
        const serverNav = document.querySelector('[data-view="servers"]');
        if (serverNav) {
            const div = document.createElement('div');
            div.id = 'modNavSlot';
            div.innerHTML = `<div class="sidebar-divider"></div><a class="nav-item" id="modNavBtn" data-view="moderation" data-label="Moderation" onclick="App.navigate('moderation')"><svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg></a>`;
            serverNav.parentNode.insertBefore(div, serverNav);
            console.log('[App] Moderation Nav-Button vor Server eingefuegt (Fallback)');
        }
    },
    ...
    initLoginHandlers() {
        ...
        // Gespeicherten User laden und Button anpassen
        this._setupQuickLogin(btnDiscord);
    },

    async _setupQuickLogin(btnDiscord) {
        try {
            const savedUser = await window.electronAPI.loadVerifiedUser();
            if (savedUser && savedUser.username) {
                // Button-Text aendern: "Als [Username] anmelden"
                btnDiscord.innerHTML = `
                    ...
                    Als ${savedUser.username} anmelden`;
                btnDiscord.addEventListener('click', async () => {
                    this.setLoginStatus('info', 'Willkommen zurueck!');
                    // Session wiederherstellen
                    AuthService.session = { token: 'saved_' + Date.now(), user: savedUser };
                    AuthService.saveSession();
                    await sleep(400);
                    this.showDashboard(savedUser);
                });
            } else {
                btnDiscord.addEventListener('click', () => {
                    this.setLoginStatus('info', 'Nutze /verify im Discord um einen Code zu erhalten.');
    ...
    async doVerify() {
        ...
        if (result.success) {
            ...
            // User persistent speichern (ueberlebt App-Neustart + localStorage-Clear)
            try { await window.electronAPI.saveVerifiedUser(result.user); } catch(e) {}
            await sleep(700);
...
const ModPanel = {
    ...
    async createModeration() {
        ...
        if (!user) { App.showNotification('Moderation', 'Bitte zuerst einen User suchen.', 'error'); return; }
    ...
    renderStaff(staff) {
        ...
        if (!list) return;
        if (!staff.length) { list.innerHTML = '<div class="mod-staff-empty">Keine Moderatoren On Duty</div>'; return; }
